...
{
    // ngx_event_timer_alarm 为 1 时，表示需要更新时间
...
ngx_event_process_init(ngx_cycle_t *cycle)
{
    ...
    // 若配置文件中设置了timer_resolution 则表明多长时间调用ngx_timer_signal_handler来更新时间
    if (ngx_timer_resolution && !(ngx_event_flags & NGX_USE_TIMER_EVENT)) {
        ...
        // setitimer 定时器 参考：http://baike.baidu.com/view/6339376.htm
    ...
    //初始化连接池
    // 按照序号,将读写时间设置到每个ngx_connection_t连接对象中，同时把这些连接以ngx_connection_t中的data成员作为next指针
    // 串联成链表。为下一步设置空连接链表做好准备
    do {
        i--;
