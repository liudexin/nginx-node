...
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
    ...
    // NGX_MAXHOSTNAMELEN 定义于ngx_config.h文件中
    ...
    // 此次copy 未用ngx_psstrdup。而是直接用ngx_pnalloc多申请了一个字节的空间
    // 是为了用ngx_cpystrn函数，将最后一个字节填充为\0
    cycle->conf_file.data = ngx_pnalloc(pool, old_cycle->conf_file.len + 1);
    if (cycle->conf_file.data == NULL) {
        ngx_destroy_pool(pool);
        return NULL;
    }
    // 将old_cycle->conf_file.data中的old_cycle->conf_file.len + 1个字节复制到cycle->conf_file.data中
    ngx_cpystrn(cycle->conf_file.data, old_cycle->conf_file.data,
                old_cycle->conf_file.len + 1);

    //配置参数设定  
    //例如: nginx -g "daemon on" 中的　daemon on
    ...
    // ngx_strlen 封装了strlen函数
    // man strlen  得知　为取得字符串的长度，但去掉最后一个\0位
    ...
    //指令结果赋值  
    // conf.ctx 这里相当于conf_ctx 的一个临时变量，可以随意的操作。但conf_ctx 却不允许随意的操作。因为这里存储了所有模块的配置信息
    conf.ctx = cycle->conf_ctx; //原来和恶心的四级指针一样
    conf.cycle = cycle;
    conf.pool = pool;
    conf.log = log;
    ...
    // 调用ngx_conf_param 这个函数，主要是完成这样的解析
    // 例如：nginx -g "daemon on"
    ...
    //这函数是解析模块的核心函数，对配置文件边解析边处理
    // conf 为ngx_conf_t 结构体
    // cycle->conf_file 保存nginx的配置文件 例如：/usr/local/sinasrv2/etc/nginx.conf
    ...
    // ngx_process 定义在  os/unix/ngx_process_cycle.c  文件中的全局变量
    // NGX_PROCESS_SIGNALLER 定义于os/unix/ngx_process_cycle.h 文件中
    // 信号处理
    // nginx -s stop|quit|reload|reopen 时 才会将ngx_process 赋值为NGX_PROCESS_SIGNALLER
    //  这个可以从ngx_get_options 函数中看到
    ...
    if (cycle->new_log.file == NULL) {
        // 定义于core/ngx_conf_file.c 文件中, error_log 文全局变量
    ...
    if (!ngx_use_stderr && cycle->log->file->fd != ngx_stderr) {
        ...
        // ngx_set_stderr 定义于 /os/unix/ngx_files.h 文件中
        // 将标准错误输出定位于cycle->log->file_>fd 文件中
...
// nginx -s stop|quite|reload|reopen 等信号处理
// 此函数主要是通过ngx_get_conf将主配置结构体ngx_core_conf_t中的nginx 的pid 文件中的pid 取出来。并
// 转换为整数。传递给ngx_os_signal_process函数处理。
ngx_int_t
ngx_signal_process(ngx_cycle_t *cycle, char *sig)
{
    ...
    //定义于 os/unix/ngx_process.c 文件中
    //pid 为 nginx 的pid
    //sig 为 stop|quit|reload|reopen 其中的一个
...
#if !(NGX_WIN32)
        if (user != (ngx_uid_t) NGX_CONF_UNSET_UINT) { //if start
            ngx_file_info_t  fi;
