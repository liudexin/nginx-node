...
// 打开nginx 错误日志文件，并将相应的信息填充到ngx_log_t结构体，并返回该结构体
// 传递的参数为nginx的安装路径
ngx_log_t *
ngx_log_init(u_char *prefix)
{
    ...
    // ngx_log 及 ngx_log_file都为ngx_log模块中的全局变量
    ...
    if (name[1] != ':') {
#else
    // 若name中存储的路径信息是完整的路径信息，则不再考虑prefix参数
    if (name[0] != '/') {
#endif
...
// 是否在config的时候已经指定了NGX_PREFIX
        ...
        if (plen) {
            ...
            // ngx_cpymem 返回的地址为name + plen ,而这个数据实际上存储在name 到name + plen -1 这一段空间上，name + plen 这个地址空间上没有数据
            p = ngx_cpymem(name, prefix, plen);

            // 若 p 这个地址从存储的数据的末尾不是以 / 结尾，则填补为以 / 结尾
            if (!ngx_path_separator(*(p - 1))) {
                *p++ = '/';
            }

            // 生存错误日志的完整路径
            // 例如：prefix 为 /usr/local/nginx/ 
            // NGX_ERROR_LOG_PATH 为 logs/error.log
            //  则完整路径为 /usr/local/nginx/logs/error.log
            //  不继续用ngx_cpymem 而用ngx_cpystrn的原因是，用ngx_cpystrn将地址空间p 的最后一位之置为\0
