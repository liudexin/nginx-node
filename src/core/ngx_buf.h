...
struct ngx_buf_s {
    u_char          *pos;       //缓冲区数据的起始位置(当buf所指向的数据在内存里的时候，pos指向的是这段数据开始的位置)
    u_char          *last;      //缓冲区数据的结束位置(当buf所指向的数据在内存里的时候，last指向的是这段数据结束的位置)
    off_t            file_pos;  //缓冲区文件数据的起始位置(当buf所指向的数据是在文件里的时候，file_pos指向的是这段数据的开始位置在文件中的偏移量)
    off_t            file_last; //缓冲区文件数据的结束位置(当buf所指向的数据是在文件里的时候，file_last指向的是这段数据的结束位置在文件中的偏移量)

    // 当buf所指向的数据在内存里的时候，这一整块内存包含的内容可能会包含在多个buf中(比如在某段数据中间插入了其他的数据，
    ...
    unsigned         in_file:1;     //文件缓冲,该buf缓存的内容在文件中
...
//  对ngx_chaint_t类型的释放，并不是真的释放了内存，而仅仅是把这个对象挂在了这个pool对象的一个叫做chain的字段对应的chain上，
//  以供下次从这个pool上分配ngx_chain_t类型对象的时候，快速的从这个pool->chain上取下链首元素就返回了，
//  当然，如果这个链是空的，才会真的在这个pool上使用ngx_palloc函数进行分配
