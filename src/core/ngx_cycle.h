...
struct ngx_cycle_s {
    ...
    ngx_str_t                 conf_file;        //配置文件 ,配置文件全路径 例如：/usr/local/sinasrv2/etc/nginx.conf
    ngx_str_t                 conf_param;       //配置参数  例如： nginx -g "daemon on"
    ngx_str_t                 conf_prefix;      //配置文件前缀，例如 若全路径为 /usr/local/sinasrv2/etc/nginx.conf ,则前缀的确认方法为:从全路径末尾开始的第一个 / 之前。 即：/usr/local/sinasrv2/etc/
