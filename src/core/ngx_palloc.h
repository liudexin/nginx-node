...
// 主要是避免重复声明，在每一个.h文件中都有类似的声明
// 参考：http://baike.baidu.com/view/1617265.htm
...
/*
 * NGX_MAX_ALLOC_FROM_POOL should be (ngx_pgesize - 1), i.e. 4095 on x86.
...
typedef struct {
    ngx_fd_t              fd;   //文件描述符，用于 ngx_pool_cleanup_file , ngx_pool_delete_file 函数
    u_char               *name;  // 文件名，用于 ngx_pool_cleanup_file 函数
    ngx_log_t            *log;
...
ngx_int_t ngx_pfree(ngx_pool_t *pool, void *p);  //释放large上的内存空间
