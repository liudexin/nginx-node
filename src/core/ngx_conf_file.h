...
#define NGX_CONF_1MORE       0x00000800 //参数为一个以上
...
struct ngx_command_s {
    ...
    ngx_uint_t            conf;         //数据存放在哪个配置中
    ngx_uint_t            offset;       //数据存放在哪个配置的那个位置中
    void                 *post;         //一般为null
};
...
struct ngx_conf_s {
    char                 *name;  //当前解析到的指令名
    ngx_array_t          *args;  //当前解析到指令的参数

    ngx_cycle_t          *cycle; //指向系统参数，在系统整个运行过程中，
                                 //需要使用的一些参数、资源需要统一的管理
    ngx_pool_t           *pool;  //内存池
    ngx_pool_t           *temp_pool; //分配临时数据空间的内存池
    ngx_conf_file_t      *conf_file; //当前解析的配置文件名称
    ngx_log_t            *log; //日志

    void                 *ctx;  //模块的配置信息, 会被赋值为ngx_cycle 结构体中的conf_ctx 用于临时变量使用。这样就可以随意的更改了，而不会影响conf_ctx
    ngx_uint_t            module_type; //当前指令的类型
    ngx_uint_t            cmd_type; //命令的类型 ,即commands 结构体中的type
...
/* 指令若在的初始值实在create_conf 中设置的
 * 在配置文件解析的过程中，若解析到相应指令，则会调用其相应的set函数对指令赋值
 * 若未在配置文件中定义，则会在配置文件解析完成之后，调用init_conf函数对此模块中的所有指令赋值。
 * ngx_conf_init_value 的意思为: 若指令只在create_conf 中初始化过。但未调用相应的set 函数对自己赋值
 * 则 在init_conf 中调用ngx_conf_init_value 对 指令赋默认值
 */
