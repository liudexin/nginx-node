...
typedef struct {
    ngx_uint_t        hsize; //hsize 为nxg_array_t 中的nalloc 

    ngx_pool_t       *pool;
    ngx_pool_t       *temp_pool;

    // 主要保存没有特殊字符的数据 如weibo.com
    ngx_array_t       keys;     // 从初始化中可以看出,keys 是一个ngx_hash_key_t 的数组。
    ngx_array_t      *keys_hash; // 从初始化中可以看出，keys_hash 也是一个数组，只是每个数组中的元素都是ngx_array类型的.
                                 // keys 和 keys_hash 之间可以通过key 关联到一起。这个可以从ngx_hash_add_key 中可以看出
                                 //  以下两组也是类似的意思

    // 主要保存前置有特殊字符的数据。如 .weibo.com *.weibo.com
    ngx_array_t       dns_wc_head;
    ngx_array_t      *dns_wc_head_hash;
    // 主要保存后置有特殊字符的数据。如 weibo.com.*
